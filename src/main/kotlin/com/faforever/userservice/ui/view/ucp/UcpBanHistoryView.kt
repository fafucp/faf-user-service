...
class UcpBanHistoryView(
    ...
    private fun createBanDetails(ban: UcpBanHistoryEntry): Component {
        ...
        val detailsContent = Div().apply {
            ...
            style.set("grid-template-columns", "1.3fr 1fr 1fr")
            style.set("column-gap", "var(--lumo-space-l)")
            style.set("row-gap", "var(--lumo-space-s)")

            // Full reason
            add(
                createDetailRow(
                    getTranslation("ucp.banHistory.details.reason"),
                    ban.reason,
                    allowWrapping = true,
                    columnSpan = 3,
                ),
            ...
            add(
                ...
                createDetailRow(
                    getTranslation("ucp.banHistory.details.issued"),
                    formatDateTime(ban.createTime),
                ),
                createDetailRow(
            ...
            if (ban.revokeTime != null) {
                add(
                    Div().apply {
                        style.set("grid-column", "1 / -1")
                        style.set("border-top", "1px solid var(--lumo-contrast-10pct)")
            ...
            ban.revokeAuthorId?.let { revokeAuthorId ->
                add(
                    createDetailRow(
                        getTranslation("ucp.banHistory.details.revokedBy"),
                        formatModerator(
                            ban.revokeAuthorUsername,
                            revokeAuthorId,
                        ),
                    ),
                )
    ...
    private fun createDetailRow(
        ...
        val labelComponent = Span(label).apply {
            ...
            style.set("margin-bottom", "0.05rem")
        }
        ...
        return Div(labelComponent, valueComponent).apply {
            style.set("grid-column", "span $columnSpan")
    ...
    private fun formatModerator(username: String?, userId: Int): String {
        return username?.let { "$it (ID: $userId)" } ?: "ID: $userId"
    }
