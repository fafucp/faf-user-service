...
class UcpSteamLinkService(
    ...
    fun buildSteamLinkUrl(userId: Int): String {
        val token = fafTokenService.createToken(
            fafTokenType = FafTokenType.LINK_TO_STEAM,
            lifetime = Duration.ofHours(6),
            attributes = mapOf(KEY_USER_ID to userId.toString()),
        )
    ...
    fun linkToSteam(parameters: Map<String, List<String>>): LinkResult {
        ...
        val userId = token?.let {
            try {
                fafTokenService.getTokenClaims(FafTokenType.LINK_TO_STEAM, it)[KEY_USER_ID]?.toInt()
            } catch (e: Exception) {
