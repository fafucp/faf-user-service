...
class UcpBanHistoryServiceTest {
    ...
    @Test
    fun mapsBanAuthorAndRevokeAuthor() {
        val ban = buildBan(
            id = 1,
            authorId = AUTHOR_ID,
            level = BanLevel.GLOBAL,
        ...
        val author = User(
            id = AUTHOR_ID,
            username = "BanModerator",
            password = "password",
            email = "ban-moderator@example.com",
            ip = null,
            acceptedTos = null,
        )

        val revokeAuthor = User(
            id = REVOKE_AUTHOR_ID,
            username = "AppealModerator",
            password = "password",
        ...
        whenever(
            banRepository.findByPlayerIdOrderByCreateTimeDesc(USER_ID),
        ).thenReturn(listOf(ban))

        whenever(userRepository.findById(AUTHOR_ID))
            .thenReturn(author)

        whenever(userRepository.findById(REVOKE_AUTHOR_ID))
            .thenReturn(revokeAuthor)
    ...
    @Test
    fun keepsModeratorIdsWhenModeratorUsersCannotBeFound() {
        val ban = buildBan(
            id = 1,
            authorId = AUTHOR_ID,
            revokeTime = NOW.minusDays(1),
        ...
        val entry = ucpBanHistoryService
            .getBanHistoryForUser(USER_ID)
            .single()
